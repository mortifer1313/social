...
export abstract class PlatformAutomation {
  ...
  protected getProxyConfig(account: SocialAccount): { server: string; username?: string; password?: string } | undefined {
    ...
    if (account.proxyUsername) {
      config.username = account.proxyUsername;
    }
    
    if (account.proxyPassword) {
      const proxyPass = process.env[account.proxyPassword] || account.proxyPassword;
  ...
  async getContextForAccount(account: SocialAccount): Promise<BrowserContext | null> {
    ...
    try {
      ...
      const proxy = this.getProxyConfig(account);
